...
def load_sovits_new(sovits_path):
    if not os.path.exists(sovits_path):
        ...
        alt_paths = [
            sovits_path.replace("GPT_SoVITS", "GPT-SoVITS/GPT_SoVITS"),
            sovits_path.replace("GPT-SoVITS", "GPT_SoVITS"),
            os.path.join(os.path.dirname(os.getcwd()), "GPT-SoVITS", "GPT_SoVITS", "pretrained_models", os.path.basename(sovits_path))
        ]
        for alt_path in alt_paths:
            if os.path.exists(alt_path):
                print(f"Found model at alternate path: {alt_path}")
                sovits_path = alt_path
                break
